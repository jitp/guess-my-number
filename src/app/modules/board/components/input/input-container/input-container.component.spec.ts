...
describe('InputContainerComponent', () => {
  ...
  it('passes the value to the ui component', () => {
    spectator = createComponent({
      providers: [
        mockProvider(BoardSandbox, { inputValue$: cold('x|', { x: 12 }) }),
      ],
    });
  ...
  it('handles the check event', () => {
    ...
    expect(spectator.inject(BoardSandbox).checkAnswer).toHaveBeenCalledWith(20);

    page.emitCheck(null);

    expect(spectator.inject(BoardSandbox).checkAnswer).not.toHaveBeenCalledWith(
      null
    );
  });
});
